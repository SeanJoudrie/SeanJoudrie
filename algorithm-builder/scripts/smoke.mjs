...
async function runFull() {
  ...
  // Results: the feed first (plan, then a shelf per topic), the clean-up below it.
  check(await waitTrue(page.getByRole('heading', { name: 'Your new feed', level: 1 })), `${name}: results open on "Your new feed"`)
  const plan = page.getByRole('heading', { name: 'Your plan this week' })
  const clean = page.getByRole('heading', { name: 'Also do these 2 things today' })
  check((await plan.count()) === 1 && (await clean.count()) === 1, `${name}: a plan, and 2 clean-up things`)
  check((await plan.boundingBox()).y < (await clean.boundingBox()).y, `${name}: the feed comes before the clean-up`)
  check((await page.getByText('Delete Game of Thrones videos from your history.').count()) === 1, `${name}: clean-up step 1 is deleting history`)
  check((await page.getByRole('checkbox').count()) === 0, `${name}: no boxes to tick`)
  ...
  const planned = Number((await page.getByText(/^Watch these \d+ videos/).innerText()).match(/\d+/)[0])
  check(planned >= 10 && (await cards.count()) === planned, `${name}: every planned video is there (${await cards.count()} of ${planned}; short: ${JSON.stringify(short)})`)
  // Each topic in the plan has its own shelf, with a surprise from somewhere else.
  const legend = await page.locator('section[aria-labelledby=plan] li').allInnerTexts()
  check(legend.length >= 4 && (await page.locator('section[id^="shelf-"]').count()) === legend.length, `${name}: a shelf for each topic in the plan (${legend.join(', ')})`)
  check((await page.locator('#shelf-wildcard a[href*="watch?v="]').count()) === 1, `${name}: the surprise is a real video`)
  // Something specific: tap a pick and it joins the plan with its own video.
  const hitch = page.locator('#shelf-classic-films').getByRole('button', { name: 'Hitchcock', exact: true })
  await click(hitch)
  check(await pressed(hitch), `${name}: a pick can be chosen`)
  check(await waitTrue(page.locator('#shelf-classic-films').getByText(/Hitchcock/).nth(1)), `${name}: the pick gets its own video or search`)
  await click(hitch)
  check(!(await pressed(hitch)), `${name}: and taken out again`)
...
async function runBoredNoKey() {
  ...
  // Channels to follow, inside each topic: 3 at a time, "not for me" with undo, others.
  const rec = page.getByRole('region', { name: /^Channels to follow for / }).first()
  const recLinks = rec.getByRole('link')
  await recLinks.first().waitFor({ timeout: 10000 })
  const before = await recLinks.count()
  check(before >= 1 && before <= 3 && (await recLinks.first().getAttribute('href')).startsWith('https://www.youtube.com/channel/UC'), `${name}: up to 3 channels per topic, linked to YouTube`)
  const firstName = (await rec.locator('li a span.truncate').first().innerText()).trim()
  const more = (await rec.getByRole('button', { name: 'Show me other channels' }).count()) > 0
  ...
  check((await rec.locator('li a span.truncate', { hasText: firstName }).count()) === 0 && (await recLinks.count()) === (more ? before : before - 1), `${name}: a hidden channel goes, and another takes its place if there is one`)
  ...
  const others = page.getByRole('button', { name: 'Show me other channels' }).first()
  if ((await others.count()) > 0) {
    const region = page.getByRole('region', { name: /^Channels to follow for / }).filter({ has: others })
    const page1 = await region.locator('li a span.truncate').allInnerTexts()
    await click(others)
    const page2 = await region.locator('li a span.truncate').allInnerTexts()
    check(page2.join() !== page1.join(), `${name}: "Show me other channels" shows others`)
  }
...
async function runQuota() {
  ...
  await page.getByPlaceholder('Search, or type your own').fill('Lego Technic')
  await click(page.getByRole('button', { name: 'Show my fix' }))
  await page.locator('a[href*="results?search_query"]').first().waitFor({ timeout: 10000 })
...
async function runTips() {
  ...
  for (const [value, host, word] of [
    ...
    await click(page.getByRole('button', { name: 'Show my fix' }))
    await page.getByRole('heading', { name: 'Do these 2 things in the app today' }).waitFor()
...
async function runBigText() {
  ...
  await primaryVisible(page, 'Show my fix', `${name}-q3`)
  await click(page.getByRole('button', { name: 'Show my fix' }))
  await page.getByRole('heading', { name: 'Your new feed', level: 1 }).waitFor()
  await screen(page, `${name}-4-results`, { phone: true })
  check(errors.length === 0, `${name}: no console errors ${errors.length ? JSON.stringify(errors) : ''}`)
  await ctx.close()
  app.stop()
