...
export const COPY = {
  ...
  q3: {
    ...
    matches: 'Tap one to pick it.',
    useOwn: (x: string) => `+ Just “${x}”`,
    notFound: 'Not in our list. Tap it above to use it anyway.',
  ...
  results: {
    ...
    leadTips: 'Search for these in the app this week.',
    ...
    cleanTitle: 'Also do these 2 things today',
    cleanTitleTips: 'Do these 2 things in the app today',
    more: (n: number) => `More ways to fix it (${n})`,
    less: 'Show less',

    watchTitle: 'Search for these this week',
    watchTips: 'Search for these in the app. Watch a few all the way through.',
    planTitle: 'Your plan this week',
    planLead: (n: number) => `Watch these ${n} videos. YouTube learns from what you finish.`,
    share: (pct: number, n: number) => `${pct}% · ${n} ${n === 1 ? 'video' : 'videos'}`,
    surpriseTitle: 'Surprise pick',
    surpriseLead: 'Something good you might not find on your own.',
    picksTitle: (x: string) => `Want something in ${x}? Tap one to add it.`,
    channelsTitle: 'Channels to follow',
    channelsFor: (x: string) => `Channels to follow for ${x}`,
    handPicked: 'Hand-picked',
    goodChannel: 'Good channel',
    popular: 'Popular this month',
    latest: (x: string) => `Latest: ${x}`,
    notForMe: (x: string) => `Not for me: ${x}`,
    hidden: (x: string) => `Got it. We won’t show ${x} again.`,
    undo: 'Undo',
    showOthers: 'Show me other channels',
    noneLeft: 'You hid all of them.',
    showHidden: 'Show them again',
