...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <article
      className="group relative overflow-hidden rounded-2xl border border-navy-800 bg-navy-900/60 transition-colors hover:border-navy-600"
    >
      <div
        aria-hidden
      ...
      <div className="p-6 sm:p-8">
        ...
        {/* [FILL: screenshot] — when project.screenshot is set, it renders here */}
        {project.screenshot && (
          <img
        ...
        <div className="mt-6 flex items-center justify-between gap-4 border-t border-navy-800 pt-5">
        ...
        >
          <div className="overflow-hidden">
            <dl className="mt-5 space-y-4 text-sm leading-relaxed">
              <CaseRow label="The problem" body={project.caseStudy.problem} />
              <CaseRow label="What I built" body={project.caseStudy.built} />
              <CaseRow label="Outcome" body={project.caseStudy.outcome} />
            </dl>
          </div>
        </div>
      </div>
...
function CaseRow({ label, body }: { label: string; body: string }) {
  return (
    <div>
      <dt className="font-display text-xs font-semibold uppercase tracking-wider text-accent">
        {label}
      </dt>
