 ...
 * 160 radial segments keep the silhouette smooth at 2× zoom (the Phase-1
 * faceting bar). The lathe axis is +Y: the dial faces +Y until the parent
 * group orients the watch. Every metal part wears the ONE case material
 * instance passed in — config swaps damp that instance, and everything
 * (lugs, crown, bracelet links) follows automatically.
 */
...
export function CaseBody({ material }: { material: MeshPhysicalMaterial }) {
  ...
  return (
    <group>
      ...
      <mesh position={[0, -5.45 * MM, 0]} rotation-x={Math.PI / 2} material={material}>
      ...
      <group position={[21.6 * MM, 0, 0]} rotation-z={-Math.PI / 2}>
        ...
        <mesh position={[0, 3 * MM, 0]} material={material}>
          <cylinderGeometry args={[3.7 * MM, 3.5 * MM, 3.4 * MM, 48]} />
        </mesh>
        <mesh position={[0, 4.9 * MM, 0]} material={material}>
...
/** Bezel silhouette in mm — sits on the case's bezel seat, domes gently
    toward the crystal opening. Its own lathe and its own material so the
    config system can drive it independently. */
...
export function Bezel({ material }: { material: MeshPhysicalMaterial }) {
  const geometry = useMemo(() => {
